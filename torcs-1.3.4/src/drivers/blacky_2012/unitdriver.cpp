  ...
  for (int L = 0; L < gNBR_RL; L++)
  {
	...
	else for (int I = 0; I < Count; I++)
    {
	  ...
	  if (LastParSeg == NULL)
	  {
	    ...
	    ParSeg->Apex = GfParmGetNum(Handle, idx,
	        (char*) PRM_APEX, (char*) NULL, (float) LastParSeg->Apex);
		ParSeg->ScaleFriction = GfParmGetNum(Handle, idx,
	...
	// Close the loop
    LastParSeg = oRacingLine[0].GetParamSeg(0);
...
//  GfOut("%d(%.0f)%d TS:%6.1f CS:%6.1f A:%g B:%g C:%g G:%d S:%g\n",
//    oIndex,Pos,oSecIndex,oTargetSpeed,oCurrSpeed,oAccel,oBrake,oClutch,oGear,oSteer);
  if (oShowIndex)
    GfOut("%d(%.0f)%d TS:%6.1f CS:%6.1f A:%g B:%g C:%g G:%d\n",
      oIndex,Pos,oSecIndex,oTargetSpeed,oCurrSpeed,oAccel,oBrake,oClutch,oGear);
//  if (oIndex == 0)
//	  GfOut("S:%9.6f\n",oSteer);
...
*/
  ...
  if (Diff > 1.0)
  {
	int	B = int(floor(oCurrSpeed/2));
	oAccel = 0;
	oBrake = MAX(0, MIN(oBrakeCoeff[B]/oInitialBrakeCoeff * Diff * Diff, oBrakeForceMax));
//    Err = 1.0 + MIN(Err,0.5);
//    oBrake = Err * oBrake;
	oLastBrakeCoefIndex = B;
	oLastBrake = oBrake;
...
double TDriver::FilterRelax(double TargetSpeed)
{
  if (!Qualifying)
  {
	if (oCar->race.timeBeforeNext > 15)
	{
      if (oCar->race.pos < 5)
	  {
        TargetSpeed *= 0.90;
...
//	  continue;
  ...
  if((CarSpeedLong > oSlowSpeed) && (Brake > 0.0))
  {
    ...
    Brake *= MAX(0.1, 0.7*cos(DriftAngle));
  }
