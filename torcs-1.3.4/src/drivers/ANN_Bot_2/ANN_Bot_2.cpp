/***************************************************************************
    ...
    email                : torcs@free.fr
    version              : $Id: ANN_Bot_2.cpp,v 1.45.2.10 2012/05/31 23:16:57 berniw Exp $
...
shutdown(int index)
{
	...
	printf("Deleting myCar.\n");
	if(myCar != NULL)
	{
		delete myCar;
	}

	printf("Deleting trackDesc.\n");
	if(trackDesc != NULL)
	{
		delete trackDesc;
	}
...
ANN_Bot_2(tModInfo *modInfo)
{
	...
	snprintf(buf, BUFSIZE, "%sdrivers/ANN_Bot_2/ANN_Bot_2.xml", GetLocalDir());
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	if (s2-s1 < TRACKNAMESIZE) {
		...
		trackname[0] = 0;
		GfError("ANN_Bot_2.cpp, initTrack, filename too long");
	}
	...
	snprintf(sstring, BUFSIZE, "%sdrivers/ANN_Bot_2/tracks/%s/car-%s-%d.xml", GetLocalDir(), trackname, carname, index);
	*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
	if (*carParmHandle != NULL) {
		GfOut("Player: %s Loaded\n", sstring);
	} else {
		snprintf(sstring, BUFSIZE, "%sdrivers/ANN_Bot_2/tracks/%s/car-%s.xml", GetLocalDir(), trackname, carname);
		*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
		if (*carParmHandle != NULL) {
			GfOut("Player: %s Loaded\n", sstring);
		} else {
			snprintf(sstring, BUFSIZE, "%sdrivers/ANN_Bot_2/car-%s-%d.xml", GetLocalDir(), carname, index);
			*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
			if (*carParmHandle != NULL) {
				GfOut("Player: %s Loaded\n", sstring);
			} else {
				snprintf(sstring, BUFSIZE, "%sdrivers/ANN_Bot_2/car-%s.xml", GetLocalDir(), carname);
				*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
				if (*carParmHandle != NULL) {
					GfOut("Player: %s Loaded\n", sstring);
				} else {
					snprintf(sstring, BUFSIZE, "%sdrivers/ANN_Bot_2/car.xml", GetLocalDir ());
					*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
					if (*carParmHandle != NULL) {
						GfOut("Player: %s Loaded\n", sstring);
					}
...
static void drive(int index, tCarElt* car, tSituation *s)
{
	...
	sensors->sensors_update();
	myCar->update(trackDesc, car, s);

	float targetAngle = atan2(myCar->destpathseg->getLoc()->y - car->_pos_Y, myCar->destpathseg->getLoc()->x - car->_pos_X) - car->_yaw;
	NORM_PI_PI(targetAngle);

	float segmentAngle = RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
	NORM_PI_PI(segmentAngle); // put the angle back in the range from -PI to PI

	//Set the gear value based on the neural network
