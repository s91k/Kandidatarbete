...
extern "C" int
human_Recorder(tModInfo *modInfo)
{
	int i;
	const char *driver;
	...
	snprintf(buf, BUFSIZE, "%sdrivers/human_Recorder/human_Recorder.xml", GetLocalDir());
 ...
 *
 * Description
 *	search under drivers/human_Recorder/tracks/<trackname>/car-<model>-<index>.xml
 *		     drivers/human_Recorder/car-<model>-<index>.xml
 *		     drivers/human_Recorder/tracks/<trackname>/car-<model>.xml
 *		     drivers/human_Recorder/car-<model>.xml
 *
 * Parameters
 *
 *
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	if (s2-s1 < TRACKNAMESIZE) {
		...
		trackname[0] = 0;
		GfError("human_Recorder.cpp, initTrack, filename too long");
	}
	...
	snprintf(sstring, BUFSIZE, "%sdrivers/human_Recorder/tracks/%s/car-%s-%d.xml", GetLocalDir(), trackname, carname, index);
	*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
	if (*carParmHandle != NULL) {
		GfOut("Player: %s Loaded\n", sstring);
	} else {
		snprintf(sstring, BUFSIZE, "%sdrivers/human_Recorder/tracks/%s/car-%s.xml", GetLocalDir(), trackname, carname);
		*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
		if (*carParmHandle != NULL) {
			GfOut("Player: %s Loaded\n", sstring);
		} else {
			snprintf(sstring, BUFSIZE, "%sdrivers/human_Recorder/car-%s-%d.xml", GetLocalDir(), carname, index);
			*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
			if (*carParmHandle != NULL) {
				GfOut("Player: %s Loaded\n", sstring);
			} else {
				snprintf(sstring, BUFSIZE, "%sdrivers/human_Recorder/car-%s.xml", GetLocalDir(), carname);
				*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
				if (*carParmHandle != NULL) {
					GfOut("Player: %s Loaded\n", sstring);
				} else {
					snprintf(sstring, BUFSIZE, "%sdrivers/human_Recorder/car.xml", GetLocalDir ());
					*carParmHandle = GfParmReadFile(sstring, GFPARM_RMODE_REREAD);
					if (*carParmHandle != NULL) {
						GfOut("Player: %s Loaded\n", sstring);
					}
...
void newrace(int index, tCarElt* car, tSituation *s)
{
	...
	if(myCar == NULL)
	{
		myCar = new MyCar(trackDesc, car, s);
	}

	//Open the file that the training data will be written to
	outputFile.open(std::string("drivers/human_Recorder/training data.txt"));
...
static void drive_at(int index, tCarElt* car, tSituation *s)
{
	...
	timeSinceLastUpdate += s->deltaTime;
	myCar->update(trackDesc, car, s);
	
	if(timeSinceLastUpdate >= 0.1)
	{
		/* steer to next target point */
		float targetAngleOut = atan2(myCar->destpathseg->getLoc()->y - car->_pos_Y, myCar->destpathseg->getLoc()->x - car->_pos_X);
		...
		//INPUT
		outputFile<<"speed "<<myCar->getSpeed()<<std::endl;
		outputFile<<"angle "<<myCar->getDeltaPitch()<<std::endl;
		outputFile<<"targetAngle "<<targetAngleOut<<std::endl;

		//Distance sensors
		outputFile<<"distR "<<sensor->getSensorOut(0)<<std::endl;
		outputFile<<"distFR "<<sensor->getSensorOut(1)<<std::endl;
		outputFile<<"distFFR "<<sensor->getSensorOut(2)<<std::endl;
		outputFile<<"distF "<<sensor->getSensorOut(3)<<std::endl;
		...
		outputFile<<"accel "<<car->ctrl.accelCmd<<std::endl;
		outputFile<<"brake "<<car->ctrl.brakeCmd<<std::endl;
		outputFile<<"gear "<<car->ctrl.gear<<std::endl;
		outputFile<<"clutch "<<car->ctrl.clutchCmd<<std::endl;
		...
		printf_s("distR %f\n", sensor->getSensorOut(0));
		printf_s("distFR %f\n", sensor->getSensorOut(1));
		printf_s("distFFR %f\n", sensor->getSensorOut(2));
		printf_s("distF %f\n", sensor->getSensorOut(3));
