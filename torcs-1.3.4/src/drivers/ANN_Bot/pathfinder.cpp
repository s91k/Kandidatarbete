...
Pathfinder::Pathfinder(TrackDesc* itrack, tCarElt* car, tSituation *s)
{
	...
	if (isPitAvailable()) {
		...
		s1 = (int) GfParmGetNum(car->_carHandle, ANN_BOT_SECT_PRIV, ANN_BOT_ATT_AMAGIC, (char*)NULL, (float) s1);
		e3 = track->getPitExitEndId();
		e3 = (int) GfParmGetNum(car->_carHandle, ANN_BOT_SECT_PRIV, ANN_BOT_ATT_FMAGIC, (char*)NULL, (float) e3);
