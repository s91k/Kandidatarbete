...
Pathfinder::Pathfinder(TrackDesc* itrack, tCarElt* car, tSituation *s)
{
	...
	if (isPitAvailable()) {
		...
		s1 = (int) GfParmGetNum(car->_carHandle, BERNIW_RECORDER_SECT_PRIV, BERNIW_RECORDER_ATT_PITENTRY, (char*)NULL, (float) s1);
		e3 = track->getPitExitEndId();
		e3 = (int) GfParmGetNum(car->_carHandle, BERNIW_RECORDER_SECT_PRIV, BERNIW_RECORDER_ATT_PITEXIT, (char*)NULL, (float) e3);
